...
function stressIntegrandsphere1DI2(yvalI2temp) !this time already integrated once in x.
	...
	call trapz1(I2IntegrandX, eta, xmin, numtrapz, stressIntegrandspher&
	&e1DI2)
...
!print *, "tempflagb",tempflagb, "yval", yval,"eta",eta
	...
	call trapz1(I2IntegrandX, eta, 0.0, numtrapz, stressIntegrand1DI2)
	cinternalcount = cinternalcount +1;
	cinterpy(cinternalcount) = yvalI2;
	cinterpx(cinternalcount) = eta;
end

function I2INtegrandY(yi)
	use globalinfo
	implicit none
	real (kind=8), intent(in) ::  yi
	real (kind=8) :: I2IntegrandY
	
	I2IntegrandY = (-3.*R*yi**2/(2.*Sqrt(xvalI2**2+yi**2)**3)+R**3*yi**&
	&2/(2.*Sqrt(xvalI2**2+yi**2)**5))*xvalI2
	!print*, "I2Y"
	return
end function I2IntegrandY

function I2IntegrandX(xi)
	use globalinfo
	implicit none
	real (kind=8), intent(in) ::  xi
	real (kind=8) :: I2IntegrandX
	
	I2IntegrandX =  (-3.*R*yvalI2**2/(2.*Sqrt(xi**2+yvalI2**2)**3)+R**3&
	&*yvalI2**2/(2.*Sqrt(xi**2+yvalI2**2)**5))*xi
	!print*, "I2x"
	return
